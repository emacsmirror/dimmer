...
### Requirement: Users can explicitly ignore a buffer
The system SHALL provide a buffer-local ignore state that prevents dimmer from processing the buffer until cleared.

#### Scenario: User marks a buffer ignored
- **WHEN** the user invokes the ignore command for the current buffer
- **THEN** the buffer is excluded from dimmer processing

#### Scenario: User clears ignore state
- **WHEN** the user invokes the unignore command for the current buffer
- **THEN** the buffer becomes eligible for dimmer processing again
